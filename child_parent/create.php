            ...
            <font size="4">
                <select name="pupil_id" id="" required>
                    ...
                    <?php while($row = mysqli_fetch_assoc($pupil_result)){ ?>
                        <option value="<?php echo $row['pupil_id'] ?>"><?php echo $row['pupil_name'] ?></option>
                    <?php
                        }?>
                </select>
            </font>
...
if (isset($_POST['submit'])) {
    // print_r($_POST);
    // die;
    $pupil_id = mysqli_real_escape_string($conn, $_POST['pupil_id']);
    $parent_id = mysqli_real_escape_string($conn, $_POST['parent_id']);

    $query = "INSERT into pupils_parents_table(pupil_id,parent_id)
    VALUES('$pupil_id','$parent_id')
    ";
    try{
        $result = mysqli_query($conn, $query);
        if ($result) {
            echo "<center><p color:'green'>Pupil , Parent assighning done successfully</p></center>";
        } else {
